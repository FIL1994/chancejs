...
test("floating() will not take fixed + min that would be out of range", t => {
  ...
  t.throws(
    fn,
    "Chance: Min specified is out of range with fixed. Min should be, at least, -900.7199254740992"
  );
});
...
test("floating() obeys the fixed parameter, when present", t => {
  _.times(1000, () => {
    ...
    let decimals = floating.toString().split(".")[1]
      ? floating.toString().split(".")[1]
      : "";
...
test.skip("set() works as expected", t => {
